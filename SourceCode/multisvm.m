classdef multisvm
    methods (Static)
        function Model=train(training, groupnames, varargin)
            ...
            %   of predictor data. Rows of TRdeAINING correspond to observations; columns
            ...
            if nsample>2
                ...
                for i=1:nsample
                    ...
                    model{i}=fitcsvm(training,classx,varargin{:});%svmtrain(training,classx,varargin{:});
                    fprintf('Multi Class SVM Model for Class Instance %d --->\n',classInstance(i))
                    disp(model{i})
                end
            else
                %svmtrain
                %svmtrain1(training,classx,varargin{:});
        ...
        function output=predict(Model,sample,varargin)
            ...
            if nsample>2
                ...
                for i=1:nsample
                    [~,threshold]=multisvm.svmclassify(model{i},sample,varargin{:});
                ...
                output=multisvm.svmclassify(model,sample,varargin{:});
            end
...
%             end
            ...
            % deal with the various inputs - commented
%             if nargin > 2
%                 if rem(nargin,2) == 1
...
%             end
            ...
            groupnames = unique(varargin{:})%svmStruct.GroupNames;
            ...
            if ~isempty(sample)
                % shift and scale the data if necessary: commented
                sampleOrig = sample;
...
%                 end
                ...
                %     try
                [outclass,val] = multisvm.svmdecision(sample,svmStruct);
                %     catch ME
        ...
        function [out,f] = svmdecision(Xnew,svm_struct)
            ...
            kfun = svm_struct.KernelParameters.Function;%svm_struct.KernelFunction;
            kfunargs = svm_struct.KernelParameters.Scale;%svm_struct.KernelFunctionArgs;
            %kfunargs{:}
        ...
        function [Model,predicted] = classify(Sample,class,SampleTest)
            ...
            predicted=multisvm.predict(Model,SampleTest);
        end
    end
end